...
class SudokuGenerator {
  ...
  late List<List<SudokuModel>> _sudoku;
  late List<List<SudokuModel>> _sudokuSolved;

  /// Unsolved Sudoku with the specified [emptySquares].
  List<List<SudokuModel>> get newSudoku => _sudoku;
  ...
  List<List<SudokuModel>> get newSudokuSolved => _sudokuSolved;
  ...
  void _fillBox(int row, int column) {
    ...
    for (var i = 0; i < 3; i++) {
      for (var j = 0; j < 3; j++) {
        ...
        _sudoku[row + i][column + j].value = number;
      }
    }
  }
  ...
  void _removeClues() {
    while (_emptySquares > 0) {
      ...
      while (_sudoku[row][column].value == 0) {
        row = _randomGenerator() - 1;
        column = _randomGenerator() - 1;
      }
      if (_uniqueSolution) {
        var backup = _sudoku[row][column].value;
        _sudoku[row][column].value = 0;
        _sudoku[row][column].isEditable = true;

        if (SudokuUtilities.hasUniqueSolution(_sudoku)) {
          _emptySquares--;
        } else {
          _sudoku[row][column].value = backup;
        }
      } else {
        _sudoku[row][column].value = 0;
        _sudoku[row][column].isEditable = true;
